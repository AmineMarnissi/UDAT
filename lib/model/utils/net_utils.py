...
def vis_detections(im, class_name, dets,bbox_gt, thresh=0.8):
    ...
    #for bb in bbox_gt:
        #bb = tuple(bb)
        #cv2.rectangle(im, bb[0:2], bb[2:4], (0, 204, 0), thickness=3)
        #cv2.putText(im, str(bb[3]-bb[1]), (bb[0], bb[1] - 5), cv2.FONT_HERSHEY_PLAIN,1.0, (0, 0, 0), thickness=2)
    return im
...
def tsne_operation():
    ...
    with open('source_features.csv', 'r') as read_obj:
        # pass the file object to reader() to get the reader object
        csv_reader = reader(read_obj)
        # Pass reader object to list() to get a list of lists
        visible_features = list(csv_reader)

        # read csv file as a list of lists
    with open('target_features.csv', 'r') as read_obj:
        # pass the file object to reader() to get the reader object
        csv_reader = reader(read_obj)
        # Pass reader object to list() to get a list of lists
        thermal_features = list(csv_reader)
    
    print("Load csv is done")
    tsne_vis = TSNE(n_components=2).fit_transform(np.array(visible_features))
    tsne_tr = TSNE(n_components=2).fit_transform(np.array(thermal_features))

    plt.close()
    plt.scatter(tsne_vis[:, 0], tsne_vis[:, 1], color='b', s=2, label='vis')
    plt.scatter(tsne_tr[:, 0], tsne_tr[:, 1], color='r', s=2, label='tr')
    #plt.title("our model t-sne")
    #plt.legend()
    ...
    plt.savefig(save_dir + 'tv_da' + '.png')
    print('Figure saved at ' + save_dir + 'tv_da' + '.png')


def heatmap_image(p,heatmap):
    img = cv2.imread(p)
